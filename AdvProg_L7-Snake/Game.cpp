...
bool Game::canChange(Direction current, Direction next) const {
	if ((current == UP || current == DOWN) && (next == UP || next == DOWN))	return 0;
	if ((current == LEFT || current == RIGHT) && (next == LEFT || next == RIGHT)) return 0;
	return 1;
}
...
void Game::nextStep()
{
	while (!inputQueue.empty()) {
		...
		inputQueue.pop(); // remove the front of input queue
        // YOUR CODE HERE
